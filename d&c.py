...
class dncCPU:
    """
    CPU player using Divide & Conquer on candidate edge subset
    ...
    def merge_sort_edges(self, edges):
        if len(edges) <= 1:
            return edges

        mid = len(edges) // 2
        left = self.merge_sort_edges(edges[:mid])
        right = self.merge_sort_edges(edges[mid:])

        return self.merge_edges(left, right)

    def merge_edges(self, left, right):
        result = []
        i = j = 0
        ...
        while i < len(left) and j < len(right):
            ...
            if self.distance_to_goal(v1) <= self.distance_to_goal(v2):
    ...
    def get_best_move(self):
        ...
        # ---- STEP 1: Candidate edge subset ----
        candidate_edges = [
            (current, n)
            for n in neighbors
            if not self.graph.nodes[n].visited
            and (current, n) not in illegal_history
        ]

        # ---- STEP 2: Fallbacks (game rule safety) ----
        ...
        if not candidate_edges:
            candidate_edges = [(current, n) for n in neighbors]

        if not candidate_edges:
            return random.choice(list(self.graph.nodes.keys()))

        # ---- STEP 3: Divide & Conquer sorting ----
        sorted_edges = self.merge_sort_edges(candidate_edges)

        # ---- STEP 4: Greedy choice ----
        _, best_move = sorted_edges[0]
        return best_move
...
class PuzzleGameGUI:
    def __init__(self, root):
        ...
        self.cpu_player = dncCPU(self.graph, self.game_state)
