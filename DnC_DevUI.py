...
class dncCPU:
    ...
    def evaluate_best_score(self, current, depth, visited_set, illegal_history):
        ...
        for nxt in candidates:
            ...
            new_visited.add(nxt)
            sub_score = self.evaluate_best_score(nxt, depth - 1, new_visited, illegal_history)
            if sub_score > best:
                best = sub_score
...
class PuzzleGameGUI:
    ...
    def create_gui(self):
        ...
        for label, node in self.graph.nodes.items():
            ...
            # store widgets for later styling (no separate btn widget)
            self.buttons[label] = {
                'frame': cell,
                'letter': lbl_letter,
                'arrow': lbl_arrow,
