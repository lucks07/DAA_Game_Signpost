...
# ====================
# CPU PLAYER (SET version)
# Divide & Conquer + DP memoization (depth-limited)
# Uses visited_set + frozenset for DP keys (instead of bitmask).
# ====================

class dncCPU:
    """
    Depth-limited DnC + DP solver using SETS.

    State:
      (cur_label, frozenset(visited_labels), depth_left)

    DnC:
      try each neighbor -> recursive subproblem

    DP:
      memoize the state -> True/False

    Depth-limited:
      depth_limit controls foresight (smaller => more mistakes)
    """
    def __init__(self, graph, game_state, depth_limit=2):
        ...
        self.labels = sorted(list(self.graph.nodes.keys()))
        self.goal_label = 'P'

        # DP memo and parent pointers
        self.dp_memo = {}   # (cur, frozenset(visited), depth_left) -> bool
        self.parent = {}    # same key -> next_label

    def visited_set_from_nodes(self):
        return {lbl for lbl, node in self.graph.nodes.items() if node.visited}

    def is_legal(self, cur, nxt, visited_set):
        if nxt not in self.graph.get_neighbors(cur):
            return False
        if nxt in visited_set:
            return False
        return True

    def can_finish_limited(self, cur, visited_fset, depth_left):
        """
        Returns True only if we can prove a valid completion within depth_left.
        Since depth is limited, if depth runs out we return False.

        Divide & Conquer:
          - Divide: split into subproblems, one per legal neighbor
          - Conquer: recurse into each subproblem
          - Combine: return True if any subproblem succeeds

        Dynamic Programming:
          - Memoize result for each (cur, visited_fset, depth_left) state
          - Avoid recomputing identical states across branches
        """
        key = (cur, visited_fset, depth_left)
        if key in self.dp_memo:
            return self.dp_memo[key]

        # Base case: all nodes visited and we're at the goal
        if len(visited_fset) == len(self.labels):
            ok = (cur == self.goal_label)
            self.dp_memo[key] = ok
            return ok

        # Depth exhausted => cannot prove completion
        if depth_left == 0:
            self.dp_memo[key] = False
            return False

        visited_set = set(visited_fset)

        # Divide: try each neighbor as a subproblem
        for nxt in self.graph.get_neighbors(cur):
            if not self.is_legal(cur, nxt, visited_set):
                continue

            new_visited_fset = frozenset(visited_set | {nxt})

            # Conquer: recurse into subproblem
            if self.can_finish_limited(nxt, new_visited_fset, depth_left - 1):
                self.parent[key] = nxt
                self.dp_memo[key] = True
                return True

        # Combine: no neighbor led to success
        self.dp_memo[key] = False
        return False

    def get_best_move(self):
        cur = self.game_state.current_position
        visited_set = self.visited_set_from_nodes()
        visited_fset = frozenset(visited_set)

        # Reset caches each CPU turn
        self.dp_memo.clear()
        self.parent.clear()

        # Try to find a provably good move within depth_limit
        if self.can_finish_limited(cur, visited_fset, self.depth_limit):
            key = (cur, visited_fset, self.depth_limit)
            if key in self.parent:
                return self.parent[key]

        # Fallback: choose any legal move randomly, excluding known failed moves
        candidates = [n for n in self.graph.get_neighbors(cur)
                      if self.is_legal(cur, n, visited_set)
                      and (cur, n) not in self.game_state.cpu_illegal_history]
        if candidates:
            return random.choice(candidates)

        # Last resort: retry any legal move ignoring history
        candidates = [n for n in self.graph.get_neighbors(cur)
                      if self.is_legal(cur, n, visited_set)]
        if candidates:
            return random.choice(candidates)
...
class PuzzleGameGUI:
    ...
    def create_gui(self):
        ...
        self._hist("CPU uses SET + frozenset DP, depth=2.", "meta")
    ...
    def cpu_turn(self):
        ...
        if success:
            self._hist(f"CPU    →  {move}  [DP:{len(self.cpu_player.dp_memo)} states]", "move_ok")
            self.flash_cpu(move)
        else:
    ...
    def show_winner(self):
        ...
        # After user dismisses the dialog, reset everything
        self.reset_game()

    def reset_game(self):
        """Tear down old state and redraw the grid for a fresh game."""
        # Cancel any pending timer
        if self.timer_id:
            self.root.after_cancel(self.timer_id)
            self.timer_id = None

        # Rebuild logic objects
        self._build_puzzle()

        # Reset all cell widgets to their default (unvisited) appearance
        for label, node in self.graph.nodes.items():
            w = self.buttons[label]
            bg = self.C['card']
        ...
        # Clear move history and write fresh header
        self.history.config(state='normal')
        ...
        # Reset DP cache display
        self.dp_cache_label.config(text="0 states")

        # Redraw everything from fresh state and restart timer
        self.update_display()
        self.timer_seconds = 0
        self.start_timer()
